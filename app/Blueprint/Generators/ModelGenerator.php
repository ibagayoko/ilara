...
class ModelGenerator extends BaseModelGenerator
{
    ...
    public function output(array $tree): array
    {
        ...
        $stub = $this->files->get($this->stubs_path . '/model/class.stub');
    ...
    protected function populateStub(string $stub, Model $model)
    {
        ...
        // $stub = str_replace('use Illuminate\\Database\\Eloquent\\Model;', 'use Illuminate\\Database\\Eloquent\\Model;' . PHP_EOL . 'use Illuminate\\Database\\Eloquent\\Relations\\BelongsTo;
        // ;', $stub);
    ...
    private function buildRelationships(Model $model)
    {
        ...
        $methods = '';
        $template = $this->getStub('belongTo');

        /** @var Column $column */
        foreach ($columns as $column) {
            $name = Str::substr($column->name(), 0, -3);
            $class = Str::studly($column->attributes()[0] ?? $name);
            $relationship = sprintf("\$this->belongsTo(\App\Models\%s::class)", $class);
    ...
    {
        return 'app/Models/' . $model->name() . '.php';
    }
    ...
    private function getStub(string $stub)
    {
        ...
        if (empty($stubs[$stub])) {
            $stubs[$stub] = $this->files->get($this->stubs_path . '/model/' . $stub . '.stub');
        }
