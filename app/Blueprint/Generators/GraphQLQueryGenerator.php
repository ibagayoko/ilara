...
class GraphQLQueryGenerator implements Generator
{
    const INDENT = '            ';
    ...
    protected function populateStub(string $stub, Model $model)
    {
        ...
        $stub = str_replace('DummyClass', $model->name().'Query', $stub);
    ...
    {
        $attrs = '';
        $attrs = str_replace('DummyGraphqlName', Str::lower($model->name()), $this->getStub('attributes'));
    ...
    {
        $type = str_replace('DummyType', "\GraphQL::type('".$model->name()."')", $this->getStub('type'));
    ...
    {
        $resolve = str_replace('DummyReturn', "\App\\".$model->name()."::where('id', '=', \$args['id'])->with(\$with)->select(\$select)->first();", $this->getStub('resolve'));
    ...
    {
        return 'app/GraphQL/Queries/' . $model->name() . 'Query.php';
    }
